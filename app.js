...
let opportunities=[]; let queue=JSON.parse(localStorage.getItem('clipQueue')||'[]'); let currentPack='';
...
async function apiFetch(url,options={},retry=true){
  ...
  if(r.status===401&&retry){
    const entered=prompt('Enter your Clipping HQ access key. This is NOT your OpenAI API key.');
...
function renderQueue(){if(!queue.length){$('#queueList').className='queue-empty';$('#queueList').innerHTML='<div class="big-icon">▣</div><h3>No posts queued yet</h3><p>Save one of today\'s scout opportunities.</p><button class="primary" onclick="switchView(\'scout\')">Open Daily Scout</button>';return} $('#queueList').className='feed full';$('#queueList').innerHTML=queue.map((x,i)=>`<article class="clip-card"><div class="clip-meta"><span>QUEUED</span><span class="risk">${escapeHtml(x.risk)} risk</span></div><h3>${escapeHtml(x.title)}</h3><p class="hook"><b>DRAFT HOOK:</b> ${escapeHtml(x.hook)}</p><div class="clip-actions"><a href="${safeUrl(x.url)}" target="_blank" rel="noopener noreferrer">Source ↗</a><button class="ai-pack" onclick="generatePack(${i})">✦ AI Post Pack</button><button onclick="removeClip(${i})">Remove</button></div></article>`).join('')}
...
window.generatePack=async(i)=>{const item=queue[i];$('#packModal').classList.add('open');$('#packModal').setAttribute('aria-hidden','false');$('#packTitle').textContent=item.title;$('#packBody').textContent='Generating your hook, script, edit plan and caption…';currentPack='';try{const r=await apiFetch('/api/draft',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(item)});if(!r.ok)throw new Error();const d=await r.json();currentPack=d.text||'Could not generate a post pack.';$('#packBody').textContent=currentPack;toast(d.mode==='openai'?'OpenAI post pack ready ✦':'Template post pack ready')}catch{currentPack='AI drafting is not available yet. Check your Clipping HQ access key and OpenAI setup.';$('#packBody').textContent=currentPack}};
