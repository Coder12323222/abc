...
// Shared by every TikTok route; keeping this outside /api avoids deploying it as a function.

export const BASE_URL='https://clipping-hq.vercel.app';
export const REDIRECT_URI=`${BASE_URL}/api/tiktok/callback`;
...
function secretMaterial(){return `${process.env.TIKTOK_CLIENT_SECRET||''}|${process.env.CLIPPING_HQ_ACCESS_KEY||''}`}
...
export function encrypt(value){
  ...
  const enc=Buffer.concat([cipher.update(Buffer.from(JSON.stringify(value))),cipher.final()]);
...
export function setTokenCookie(res,tokens){
  ...
  res.setHeader('Set-Cookie',`chq_tiktok=${value}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=15552000`);
}
...
export async function refreshIfNeeded(req,res){
  const data=decrypt(cookies(req).chq_tiktok||'');
  if(!data)return null;
  ...
  if(!r.ok)return null;
  const next={...data,access_token:d.access_token,refresh_token:d.refresh_token||data.refresh_token,expires_at:Date.now()+(d.expires_in||86400)*1000};
