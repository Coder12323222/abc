...
function authorized(req){const expected=process.env.CLIPPING_HQ_ACCESS_KEY;if(!expected)return true;return req.headers['x-hq-key']===expected}
...
function extractGemini(data){return (data?.candidates?.[0]?.content?.parts||[]).map(p=>p?.text||'').join('\n').trim()}
async function tryGemini(prompt){
  ...
  const data=await r.json();if(!r.ok)throw new Error(data?.error?.message||`Gemini error ${r.status}`);return extractGemini(data)
}
...
export default async function handler(req,res){
  ...
  if(process.env.GEMINI_API_KEY){try{const text=await tryGemini(prompt);if(text)return res.status(200).json({mode:'gemini',text})}catch(e){}}
  if(process.env.OPENAI_API_KEY){try{const text=await tryOpenAI(prompt);if(text)return res.status(200).json({mode:'openai',text})}catch(e){}}
  return res.status(200).json({mode:'template',warning:'AI providers unavailable; used template mode.',text:fallback(item)})
}
